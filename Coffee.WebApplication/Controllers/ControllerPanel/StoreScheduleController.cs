...
namespace Coffee.WebApplication.ControllerPanel.Controllers
{
    public class StoreScheduleController : Controller
    {
        ...
        {
            var storeScheduleModel = new StoreScheduleModel();
        ...
        public IActionResult Save( StoreScheduleModel storeScheduleModel, Guid id)
        {
            ...
            {
                if (storeScheduleModel.StartTime.TimeOfDay >= storeScheduleModel.EndTime.TimeOfDay)
                {
                    ModelState.AddModelError("EndTime", "Invalid time period.");
                    return View(actionName, storeScheduleModel);
                }
